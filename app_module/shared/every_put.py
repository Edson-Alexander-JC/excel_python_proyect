...
class EveryPut:
    ...
    def print_input(self, item: InputItem):
        ...
        if item.unit != "":
            ...
            with col1: value = self.def_inputs(item)
            with col2:
                st.markdown(f"""<div 
                    style='
                        height:60px;
            ...
            value = self.def_inputs(item)
        return value
    
    def print_output(self, item: InputItem, 
                     padding=False,h_aling="",v_aling=""
        ):
        valor = item.value
        unidad = item.unit
        label = item.label
        ...
        v_padd = "0px"
        if padding: v_padd = "28px 0px"
        h_css = ""
        if h_aling != "": h_css = f"""align-items:{h_aling};"""
        v_css = ""
        if v_aling != "": v_css = f"""justify-content:{v_aling};"""
        #div container
        div = f"""
        <div style="
            display:flex;
